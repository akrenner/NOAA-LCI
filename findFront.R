...
poT@data <- poT@data [,names (poT@data) %in% c("Match_Name", "timeStamp"
                                             , "SST", "aveTemp", "SSS"
...
tFront <- data.frame (levels (tranS), matrix (nrow = length (levels (tranS))
                                   , ncol = ncol (poT)-1))
names (tFront) <- c ("tranS", "mDist", names (poT@data)[3:ncol (poT@data)])
...
for (i in 1:length (levels (tranS))){
    ...
    if (nrow (trD) > 3){
        ...
        # cat ("\n\n, transect = ", i, "###")
        tDist <- gDistance (trD [1,], trD, byid = TRUE)/10e3 # km not m
        tFront$mDist [i] <- max (tDist)
        for (j in 3:ncol (poT)){
            fDistInt <- data.frame (dDist = diff (tDist [,1]))
            fDistInt$slope <- abs (diff (trD@data [,j]) / fDistInt$dDist)
         #   print (j)
            if (all (is.na (fDistInt$slope))){
                ...
                plot (tDist, trD@data [,j], axes = FALSE, ylab = names (trD)[j]
                    , type = "l"
                    ## , ylim = range (poT@data [,j])
                      )
                axis (2)        
...
save.image ("~/tmp/LCI_noaa/cache/frontEnd.RData")
# load ("~/tmp/LCI_noaa/cache/frontEnd.RData")
...
for (i in 1:length (levels (tranCat))){
   ...
   # par (mfrow = c(9,1)) #, mar = c(3,4,0,1))
    ...
    for (j in 3:ncol (sumF)){
        hist (sumF [,j]
            , main = names (sumF)[j]  # paste (levels (tranCat)[i], names (sumF)[j])
            , xlim = c(0, max (sumF$mDist, na.rm = TRUE)*1.1)
              , xlab = "distance [km] East to West or North to South"
              )
