...
sampleDF <- data.frame (expand.grid(month=1:12, year=2017:2021))  ## manual set range!
...
xT <- as.matrix (sampleDF [,3:ncol (sampleDF)])
rownames (xT) <- apply (sampleDF[,1:2], 1, paste, collapse="-")
...
axis (3, at=1/length (sType)*1:length (sType)
      , labels = sType
      )
# axis (2, labels = )
