...
load ("~/tmp/LCI_noaa/cache/ctdwallAnomalies.RData")   # from CTDwall_anomalies.R
...
for (ov in oceanvarC){  # ov = OceanVariable (temp, salinity, etc)
  for (transectN in transectS){  # tn: transect
    ...
    ## doubly-used stations -- flex them
    ...
    poAll$Transect <- factor (poAll$Transect, levels = transectS)
