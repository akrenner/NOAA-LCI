```{r}
#| echo: false

## align wd with html dir
# setwd ("~/myDocs/amyfiles/NOAA-LCI/MonthlyUpdates")
## can this from from standard NOAA-LCI ?

## bathymetry to plots
## fix alignment of contour lines
## sob: fix rainfall units


## workflow:
## connect to VPN (NCCOS-West for SWMP download)
## runAll.R to process last survey and download latest weather
## pre-pub update: run AnnualStateOfTheBay.R 
## run all in this script
## render this script to html. Copy into gmail and check for errors
## re-render. Print to PDF
## publish this quatro
## copy published website and paste into gmail
## attach pdf version
## mail-merge with distribution list
## delete unsubscribe button


## next time: new structure. First: temp and salinity and stratification. 
## then longt-term trends and anomalies
## then Biology. 


## clear the slate
rm(list = ls())
# require("packrat")
# require("rsconnect")
require("quarto", quietly = TRUE)
suppressMessages(qV <- quarto_binary_sitrep())
if (!qV) {
  stop("Quarto binary not found. Please install Quarto.")
}
unlink("MonthlyUpdates/media/", recursive = TRUE)


## fetch data from notebook export and CTD processing
load("~/tmp/LCI_noaa/cache/CTDcasts.RData")  # from dataSetup.R -- contains physOc, raw CTD profiles
physOc$isoDate <- as.Date(physOc$isoTime)
## find survey, not just last date
physL <- subset(physOc, as.numeric(format(physOc$isoTime, "%Y")) ==
  max(as.numeric(format (physOc$isoTime, "%Y"))))
physL <- subset(physL, as.numeric(format (physL$isoTime, "%m")) ==
  max(as.numeric(format(physL$isoTime, "%m"))))
rm(physOc)

# minT = min (c(10.5, 10.6, 9.8, 11))
# maxT = max (c(13, 15, 14, 12))
# sDate = as.Date (Sys.Date())
# sDate = as.Date ("2024-08-26")
sDate <- max(physL$isoDate)
dateAN <- format(sDate, "%d %b %Y")


sY = as.numeric(format(sDate, "%Y"))
sM = month.name[as.numeric (format(sDate, "%m"))]
sSlot = paste0(sM, ", ", sY)
# sSlot <- "June, 2025"

crew = c("Kris","Martin", "Ross") |> sort()  ## pull from notes
crewT = paste0(c(paste0 (crew [1:(length(crew) - 1)], collapse = ", "),
  paste0(crew [length (crew)])), collapse = ", and ")


tsal <- with(physL, data.frame(temSal = c(min(Temperature_ITS90_DegC, na.rm = TRUE),
  max(Temperature_ITS90_DegC, na.rm = TRUE),
  min(Salinity_PSU, na.rm = TRUE),
  max(Salinity_PSU, na.rm = TRUE),
  min(Chlorophyll_mg_m3, na.rm = TRUE),
  max(Chlorophyll_mg_m3, na.rm = TRUE)),
stn = c(Match_Name [which.min(Temperature_ITS90_DegC)],
  Match_Name [which.max(Temperature_ITS90_DegC)],
  Match_Name [which.min(Salinity_PSU)],
  Match_Name [which.max(Salinity_PSU)],
  Match_Name [which.min(Chlorophyll_mg_m3)],
  Match_Name [which.max(Chlorophyll_mg_m3)]),
depth = c(Depth.saltwater..m. [which.min(Temperature_ITS90_DegC)],
  Depth.saltwater..m. [which.max (Temperature_ITS90_DegC)],
  Depth.saltwater..m. [which.min (Salinity_PSU)],
  Depth.saltwater..m. [which.max (Salinity_PSU)],
  Depth.saltwater..m. [which.min (Chlorophyll_mg_m3)],
  Depth.saltwater..m. [which.max (Chlorophyll_mg_m3)])
))
tsal$stn <- gsub("_", "-", tsal$stn)
tsal$temSalR <- round(tsal$temSal, 1) |>
  format(nsmall = 1)
tsal$depthT <- ifelse(tsal$depth > 5, "depth", "surface")
# tsal$region <- tsal$stn
# tsal$region <- gsub ("AlongBay_[7:13]", tsal$region, "inner bay")
# tsal$region <- gsub ("AlongBay_[1:6]", tsal$region, "outer bay")
# tsal$region <- gsub ("T4_[1:10]", tsal$region, "outer bay")
# tsal$region <- ifelse (tsal$region %in% (c("outer bay", "inner bay")), tsal$region, "Cook Inlet")


## to-do:
## tweak title layout : remove "author" and "affiliation"
## add date?
## add background graphics, NCCOS logo



## ----------------------------------------------------------------------
## copy files for figures

# Struggling with path settings -- circumvent this issue by copying relevant file to local directory

## find files
## construct mark-down code (all sections of the day, monthly / quarterly)
dest <- "media/CTDsections/sectionImages/"
dest <- "media/"
destR <- paste(getwd(), dest, sep = "/")


dir.create(destR, showWarnings = FALSE, recursive = TRUE)
src <- "~/tmp/LCI_noaa/media/"

# pick transects from last survey; flexible as to monthly/quarterly
fl <- list.files(paste0 (src, "CTDsections/sectionImages"), pattern = "*.png")
flC <- grep(substr(sort(fl, decreasing = TRUE)[1], 1, 7), fl, value = TRUE)  # pick last date

# lastDate <- "2025-01-14"
# lastDate <- substr(sort(fl, decreasing=TRUE)[1], 1,10)
lastDateT <- format(sDate, "%m/%d/%Y")  ## US style: 04/16/2025
lastDateT <- format(sDate, "%d %B %Y")  ## 16 April 2025


for (i in seq_along(flC)) {
  cS <- file.copy(from = paste0(src, "CTDsections/sectionImages/", flC [i]),
    to = paste0(destR, "/Section_", substr (flC [i], 9, nchar (flC [i]))),
    overwrite = TRUE)
}


## Chlorophyll
# dest="media/CTDsections/time-sections/"
# dir.create(dest, showWarnings=FALSE, recursive=TRUE)
flS <- list.files(paste0("~/tmp/LCI_noaa/CTDsections/time-sections", dest), pattern = "*.png")
cS <- file.copy(from = paste0 ("~/tmp/LCI_noaa/", dest, flS)
  , to = destR, overwrite = TRUE)
TStempSurface <- paste0(destR, "5-TempSurfaceTS.png")
TSsalinitySurface <- paste0(destR, "5-SalSurfaceTS.png")
TSchlorophyll <- paste0(destR, "3-9_6-chlorophyll-climatology.png")
TS_tsprofile <- paste0(destR, "2-9_6-TSprofile.png")


## NOAA weather -- replace from rNOAA

## signature data-set graphs
# check that updates were successful!
cS <- file.copy(from = "~/tmp/LCI_noaa/media/CTDsections/time-sections/3-9_6-chlorophyll-anomaly.png"
  , to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/CTDsections/time-sections/5-SalDeepTS.png"
  , to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/CTDsections/time-sections/5-TempSurfaceTS.png"
  , to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/CTDsections/time-sections/5-T9-6_TempDeepTS.png"
  , to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/CTDsections/time-sections/2-9_6-TSprofile.png"
  , to = destR, overwrite = TRUE)
## State-of-the-bay
#    cS <- file.copy (from="~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-airTemp-LE.png", to=destR, overwrite=TRUE)
#     cS <- file.copy (from="~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-precip-AP.png", to=destR, overwrite=TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-Temp-Deep-Homer.png"
  , to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-salinity31-d.png"
  , to = destR, overwrite = TRUE)
#    cS <- file.copy (from="~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-chlorophyll.png", to=destR, overwrite=TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/StateOfTheBay_monthly/CTD_AlongBay_8_dayTempBottom41.png", to = destR, overwrite = TRUE)
cS <- file.copy(from = "~/tmp/LCI_noaa/media/StateOfTheBay/update/sa-waves.png", to = destR, overwrite = TRUE)



# email to:
#   Dominic Hondolero <dominic.hondolero@noaa.gov>, Karyn DeCino <karyn.decino@noaa.gov>, Kris Holderied <kris.holderied@noaa.gov>, Paul Cziko <paul.cziko@noaa.gov>, Reid Brewer <reid.brewer@noaa.gov>, Ross Whippo <ross.whippo@noaa.gov>,
# Katherine Schake <kschake@alaska.edu>, Kim Schuster <kkschuster@alaska.edu>, Lauren Sutton <lsutton7@alaska.edu>,
# Arthur Kettle <arthur_kettle@fws.gov>

## use GMail's mail merge, pulling addresses from this spreadsheet:
## 



rm(fl, flS, dest, destR, cS)

## 1. wrap-up text
## 2. replace graphs: long TS of temp/Salinity/Chlorophyl with SoB-style annual plot

#  <!-- markdown:  -->
#  <!--   wrap: 72 -->
# title-block-style: none
# in terminal: quarto render --to pdf
# no references to R code in title section!
```

---
title: "Monthly Kachemak Bay report, September 2025"
author:
  - name: "Martin Renner"
    email: martin.renner@noaa.gov
    affiliation:
      - name: CSS, under contract to NOAA/NCCOS Kasitsna Bay Lab
  - name: "Kris Holderied"
    affiliation:
       - name: NOAA/NCCOS Kasitsna Bay Lab
format: html
editor: visual
# title-block-style: none
# in terminal: quarto render --to html
# options: html, pdf or docx, email
---

## Summary

Seasonal warming is now expanding into mid-water layers. A warm-water anomaly that started at the beginning of the year, is ongoing throughout the water column. A low salinity anomaly that also started this year, was also ongoing and may now be the longest since 2012. Chlorophyll concentrations were highest in the outer bay, but stayed below the seasonal mean. 
<!-- 2-3 sentences -->

## Conditions

This month, Hans, Kris, Martin, and Ross conducted the extended survey, which included T9 across the bay from the Homer Spit and the AlongBay transect on 24 and 25 September. <!-- `{r} dateAN` , -->
Sea state during this survey varied from Beaufort 1 to Beaufort 4. 
Air temperatures at on the spit were above the seasonal mean, approaching the 90th percentile, for much of this summer, including early September. Temperatures have now dropped not only seasonally, but are also approaching the long-term mean again (@fig-Temp_Airport). Precipitation has been well below the long-term mean since mid-June. While September has been wetter than August, precipitation is still below the long-term mean. On a larger scale, much of the North Pacific, has been in heat wave conditions this month, remeniscent of the 'blob' of 2014-16, but bigger; in fact only a few spots on the globe experience a negative sea surface anomaly conditions at the moment. 

Kachemak Bay National Estuarine Research Reserve (KBNERR) detected an *Alexandrium* bloom in early September, a potentially harmful dinoflagelate. Testing of shellfish has since revealed toxin levels up to twice the regulatory limit for human consumption ([AOOS](https://ahab.aoos.org/hab-alerts/)). The last time similar densities of *Alexandrium* were seen in Kachemak Bay was in 2016, during the extreme North Pacific marine heat wave. 

<!-- In addition to these extremes, it has also been an extreme summer in terms of wave heights in Kachemak Bay. While wave heights would not have been unusual in winter, for the summer season, they were unprecedented (@fig-waves). At the same time, wind speeds stayed within normal ranges.  -->

<!-- ![Waves at the mouth of Kachemak Bay, as measured by an AOOS operated wave buoy](./media/sa-waves.png){#fig-waves} -->

![Recent temperature and precipitation at Homer Airport, compared to their seasonal means.](./media/sa-airTemp-LE.png){#fig-Temp_Airport}

<!-- While within normal limits, it has been a cool and wet June so far. Average air temperature at Homer Airport were still below 10 °C [50  °F]. Conditions during the survey were choppy in the outer bay ( \< beaufort 5, with short steep waves up to 1.2 m, leading us to skip station AlongBay 1), but were predictably calmer in the inner bay. -->
<!-- `{r} crewT` -->
A complete set of up-to-date graphs can be found on the Google Drive: [section plots](https://drive.google.com/drive/folders/1PCXyrK7tJ897bqqUPFcmGnjLAkGfi5So?usp=drive_link) and [time sections](https://drive.google.com/drive/folders/1bkBPSBfvKjcgkOtikhIfLPOMwjgziJ8t?usp=drive_link).

## Physical oceanography

### Temperature

Recorded water temperatures during this survey ranged from a minimum of `{r} tsal$temSalR[1]` °C at station `{r} tsal$stn[1]` (`{r} tsal$depth[1]` m depth), in the outer bay near Homer Spit, to a maximum of `{r} tsal$temSalR[2]` °C at `{r} tsal$stn[2]` (`{r} tsal$depth[2]` m depth) in Cook Inlet, off Nanwalek. The falling air temperatures left their mark. Surface water temperatures in the inner bay began to equalize with midwater temperatures, which still carried heat from the summer. Even the coldest waters at the bottom of Kachemak Bay were significantly above the 8.5 °C thought to be necessary for sustaining an *Alexandrium* bloom. 

A temperature (and salinity) front, was found in lower Cook Inlet, between Flat Island and Nanwalek, reaching from the surface all the way to the bottom (@fig-tSections_AB). The same front is visible on a satelite data, showing a tongue of warm water from coastal waters, extending across Kennedy Entrance towards Cape Douglas (@fig-SST_GOA). Interesting temperature patterns were apparent along T9 and T4 (@fig-tSections). While the bottom half of the water column was still layered from a cool bottom waters to warmer mid-waters, surface waters on T9 and the south end of T4 had cooled to be already colder than mid-water layers. 


::: {#fig-tSections}
![AlongBay Transect](./media/Section_T-AlongBay.png){#fig-tSections_AB}

![T9 Transect, across-bay, Homer Spit to China Poot Bay](./media/Section_T-9.png){#fig-tSections_T9}

![T4 Transect, across-bay, Bluff Point to Barabara Point](./media/Section_T-4.png){#fig-tSections_T4}

Longitudinal and cross-sections of main parameters.
:::




![Sea surface temperature in the Gulf of Alaska, based on satellite remote sensing](media-2025-09/alaska.fc.gif){#fig-SST_GOA}

<!-- ![Sea surface temperature in the Gulf of Alaska, based on satellite remote sensing](https://www.ospo.noaa.gov/data/sst/contour/alaska.fc.gif){#fig-SST_GOA} -->

The warm water anomaly that started here at the beginning of the year, was ongoing, at least in the deepwater (@fig-TempDeep, @fig-TSprofile_T9). Near-surface waters, as may be expected from the cooling air temperatures (@fig-Temp_Airport) were more in line with what is seasonally expected, as indicated by the Homer SWMP station, operated by the KBNERR (@fig-SWMP-homerdeep-temp). Compared to previous warm water events here, the current anomaly is less severe but has now been going on for a long time. 

<!-- Since the beginning of the year, water temperatures have been warmer than the seasonal mean, and this anomaly continued in August, but was diminished compared to previous months (@fig-TempDeep). The warm anomaly was also seen at the Homer SWMP station, operated by the KBNERR (@fig-SWMP-homerdeep-temp), and most pronounced above the pycnocline (@fig-TSprofile_T9). The anomaly about 1 °C above the long-term average, and remained near the 90th percentile.  -->

<!-- add sob plot on air temperature -->

![Variability of bottom-water temperature at T9-6 compared to the seasonal long-term mean.](./media/5-T9-6_TempDeepTS.png){#fig-TempDeep}

![Water temperature at the Homer Harbor SWMP station, at approximately 7 m depth.](./media/sa-Temp-Deep-Homer.png){#fig-SWMP-homerdeep-temp}

![Deep water temperature at in the inner baay at station AlongBay-8 (deeper than 15 m).](./media/CTD_AlongBay_8_dayTempBottom41.png){#fig-DeepWaterTemp}



![Long-term temperature and salinity profiles of T9-6 and their respective anomalies.](./media/2-9_6-TSprofile){#fig-TSprofile_T9}

<!-- More figures: signature data = time series. Start with figures as-is, then elaborate on them (improving on anomaly plots). -->

<!-- Add map with station numbers -- schematic -->



### Salinity

<!-- also cover density and buoyancy here -->

Salinity values ranged from `{r} tsal$temSalR[3]` PSU at `{r} tsal$stn[3]` (`{r} tsal$depth[3]`m depth) in the inner bay to `{r} tsal$temSalR[4]` at `{r} tsal$stn[4]` (`{r} tsal$depth[4]`m depth) in the outer bay near Homer Spit. While the surface waters have cooled, compared to last month, a salinity gradient maintains a stable, stratified water column in the inner, and parts of the outer bay (@fig-tSections_T9, @fig-tSections_T4). 

The across-transects T9 and, especially, T4 show a north-sloping gradient of this stratification with the with a thicker layer of fresher water near the northern shore. This is consistent with freshwater exiting Kachemak Bay in a narrow band along the north shore, following the general counter-clockwise currents of Kachemak Bay. 

Since the beginning of the year, salinities at the mid-bay station T9-6 have been lower than the seasonal mean (@fig-SalDeep), particularly in waters below 25 m (@fig-TSprofile_T9). This anomaly has come to an end, at least for now, as we saw a slight reversal. 

![Variability of deep-water salinity at T9-6 compared to the seasonal long-term mean.](./media/5-SalDeepTS.png){#fig-SalDeep}

### Turbidity

While no areas of high turbidity were encounted during the survey, waters in the outer bay were noticably clear. The turbidity signal differed noticably from the spatial patterns of temperature and salinity. Turbidity was low throughout the outer bay and into Cook Inlet. Highest turbidity values were found near the surface in the inner bay as well as near the bottom of the inner bay (@fig-tSections_AB). 



## Biological conditions

### Phytoplankton

There were two areas of high chlorophyll concentrations: in surface waters of the inner bay, as is common during summer. A second, more unusual area of high activity was at the southwestern end of the extended AlongBay transect, in Cook Inlet off Nanwalek (@fig-tSections_AB), coinsiding with warmer temperatures and lower salinity. Interestingly, a plankton sample from this station contained Pseudo-nitzschia, which was abundant in Kachemak Bay in July, but was not expected this late in the year (Rose Masui, KBERR, pers. com.). Overall highest chlorophyll concentrations were  


Chlorophyll concentrations observed with the CTD fluorometer were below the seasonal mean for most stations (e.g. @fig-chlorophyll). Highest concentrations: 
`{r} tsal$temSalR[6]` mg m^-3 at `{r} tsal$stn[6]` (`{r} tsal$depth[6]` m depth).
<!-- Highest concentrations were observed in the inner bay in a thin layer immediately below the pycnocline (@fig-tSections_AB).  -->

![Trends and seasonal anomaly of Chlorophyll concentrations at T9-6 (average from surface to 85 m depth)](./media/3-9_6-chlorophyll-anomaly.png){#fig-chlorophyll}

<!-- ![Time section of Chlorophyll concentrations at T9-6.](./media/3-9_6-chlorophyll-timesection.png){#fig-chlorophyllsection}  -->



### Zooplankton

Echo sounder returns were weak to moderate at a few stations. No samples were taken. 
<!-- Backscatter? Species composition? -->

### Wildlife

A humpback whale was seen in the outer bay, as were several hundret red-necked phalaropes, the later foraging along convergence lines. Fork-tailed storm-petrels were seen both in the inner and the outer bay. While no sooty shearwaters were observed, several hundret short-tailed shearwater were off Point Pogibshi, with smaller numbers throughout the outer bay. Several horned puffins, rarely seen in Kachemak Bay during summer, were also off Point Pogibshi.

### Mailing list

Please email Martin at martin.renner\@noaa.gov if you would like to be added to or removed from this mailing list.
